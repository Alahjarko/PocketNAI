...
## 二十八、多角色 / 超分 / 账单 / 多账号 / 分享与批量（2026-09-18）

### 28.1 背景
用户在同期让另一个编码助手（Gemini）完成了路线图里的几件大件；实现未提交即交接。
本机复核动作：通读关键 diff（凭据、数据库迁移、请求构造、付费链路）、跑单测与构建、
装机走查免费路径（多角色编辑块、画廊多选与分享、超分确认框、账号管理、图片清理、
消耗流水），并补齐 AGENTS.md 与本节。
...
- **多账号**：密文存 `account_<id>_*`，但**顶层兼容键继续同步写**
  （旧安装升级时靠它读凭据）；三个"不得更改"的常量未动。
  接口里的多账号默认实现是**假实现**（返回 false/空）——已在 AGENTS.md 写明禁止依赖。
- **分享 / 批量 / 清理**：分享只暴露 FileProvider 的 `files/generations/`；
  批量删除与"清理生成图片"只动本地副本（默认保留收藏；已存相册的副本不受影响）。

### 28.3 复核结论
- 单测全绿、构建通过；数据库迁移 v6→v7 只建新表，符合红线。
...
### 29.1 背景与需求
用户手里有一批商业 SOCKS5 节点（100 个端点里 95 个可用；实测该服务**只开 SOCKS5**，
HTTP 代理协议全部不通）。希望：
- 应用内置"公益代理"开关，分发给朋友时免配置即可连上 NovelAI；
- 每日 750 MB 限额、超限提示（平时不打扰）；
- **"3 秒内连不通就自动换节点"**；
- 凭据**不能泄露**。

### 29.2 设计与取舍
- **节点清单加密进 assets**：AES-256-GCM（`scripts/encrypt-proxy-nodes.mjs` 生成，
  密钥派生自代码内固定串）。挡"随手提取"，挡不住专业逆向 —— 已在 AGENTS.md 里写明
  边界，并建议把真正的用量上限放到代理服务商后台；凭据按"可轮换"对待。
- **只影响 NovelAI**：检查更新与 APK 下载走直连（用户要求，也省代理流量）。
- **"3 秒不通就换"** = 代理 client 的 `connectTimeout(3s)` + 换节点重试（≤3 次）。
- **重试的安全红线**：只有"代理连接失败（请求未发出）"或"幂等请求（GET/HEAD）"才换节点重试；
  POST（生成、登录）绝不重发 —— 用方法区分，而不是猜"请求发没发出去"。
- 流量用 `EventListener` 的 body 计数（流式/未知长度都覆盖），只挂在代理 client 上；
  超限由拦截器直接拒绝并映射成 `PROXY_QUOTA_EXCEEDED`。

### 29.3 排障记录（三个真实坑，全部靠对照探针 + DEBUG 结构日志定位）
1. **OkHttp 的 `proxyAuthenticator` 管不了 SOCKS**：它只处理 HTTP 代理的 407；
   SOCKS5 用户名/密码走**全局 `java.net.Authenticator`**（JDK Socket 实现内部调用）。
   不注册 → `SOCKS : authentication failed`，请求快速失败、无日志、流量统计不动。
2. **Android 的 libcore 把 SOCKS 认证的 `requestorType` 标成 `SERVER`**（不是 JDK 的
   `PROXY`）—— 按类型过滤会把凭据吞掉。最终实现按**端口匹配当前代理端点**回答认证。
3. **OkHttp 的 RouteDatabase 会记住失败的代理路由**：同一个 client 上第一次失败之后，
   后续请求会被短路（表现为"一失败就永远失败"）。换节点重试天然绕开；
   这也是"失败就换"设计的额外收益。
4. 附带实测数据：模拟器到节点建连 1.8–3.3 秒（含 TLS），**3 秒会误杀正常连接**，
   首次请求靠 1–2 次重试成功；真机+海外代理通常 1–2 秒。当前尊重用户的"3 秒"要求，
   若真机反馈频繁重试再议调大。

### 29.4 验证
- MockWebServer 单测覆盖配置解析与额度常量；`SocksProxyProbeTest`（androidTest）
