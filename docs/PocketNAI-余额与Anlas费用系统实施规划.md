...
| 3 费用计算器 | ✅ | 四态 + 官方免费单张规则 + 官方付费公式（`NovelAiPaidAnlasFormula`，2026-09-14 反解官方前端） |
...
> **2026-09-14 更新**：本节多数条目已被"反解官方前端"取代，逐条现状见文末附录与
> 《技术决策记录》第十六节。下面保留原文，作为当时的判断记录。

1. ~~**付费定价矩阵未校准**~~ **已完成**：改为反解官方前端 bundle，落地为
   `NovelAiPaidAnlasFormula`（`supports()` 对 V4.5 / V5 均返回 true），
   §10.3 的人工矩阵不再需要执行。
2. **字段映射待与网页对照**：观测到的 407 落在"订阅池"，而账号所有者描述为"买了积分"，
   需要按 §13 阶段 0 第 3 项核对一次。
3. ~~**订阅等级整数待与网页订阅名称对照**~~ **已完成**：`0/1/2/3 → 无/Tablet/Scroll/Opus`
   从官方前端 bundle 反解确认；另有 `accountType` 枚举与官方订阅判据
   （`active` 字段不参与判断）。设置页另有手动覆盖作为兜底。
4. **`usage.percent` 的方向待确认**：观测到 0% 且"约 2 小时恢复 1%"，
   与"当前额度百分比"的解释一致，但仍建议在网页上对照一次。
5. ~~**参考图附加费待确认**~~ **已确认**：官方文档写"每张 5 Anlas"、
   官方前端是 `5 × 张数 × 输出张数`，2026-09-14 的余额观测（407 → 402）与之吻合。
   同一轮里还确认了 Vibe 的 `>4 张每张 2` 与"编码一次 2 Anlas"。

---

## 附：本文 §10 的校准流程已被官方前端反解取代（2026-09-14）

§10 的前提是"官方没有公开报价接口，只能人工读网页费用标签"，这个前提**不成立**：
官方网页前端的计价函数、免费规则、附加费规则全都在客户端 JS 里，比人工读标签更精确。

实际采用的做法是反解官方前端 bundle（方法与证据链见《技术决策记录》第十六节），
并以两个可当场手算的样本交叉验证：`1024×1024 @28 步 → 20 Anlas`、
`832×1216 @23 步 → 17 Anlas`（后者与社区长期流传的"默认一张约 17 Anlas"一致）。

因此：

- **§10.3 的最低校准矩阵不再需要执行**，不必去网页上逐格记录费用标签；
- **§10.4 的"公式进入代码的门槛"仍然有效**，只是"校准样本"换成了官方前端的中间结果；
- **§10.5 的 V5 百分比仍然是"不预测具体百分比"** —— 官方前端也只显示额度条，不给单次消耗量。

另外，§10.1 里"不得根据旧版社区库直接照抄公式"这条纪律现在有了更好的答案：
不用抄社区库，直接抄官方前端。两者的关键差别（社区库把 V4.5 当成老模型用指数式，
官方前端对 V4.5/V5 用的是线性式并给 V5 乘 1.5）已经体现在实现里。
