...
- **费用**：Vibe 的价格尚未确认，因此只要挂了它，按钮上就是`费用待确认`。
...
| 状态 | 按钮上 | 说明 | 什么时候出现 |
|---|---|---|---|
| 免费 | `免费` | 预计不消耗 Anlas | 官方免费条件成立，或实测确认的组合（含图生图） |
| V5 额度 | `0 Anlas` | 将消耗 V5 免费额度 | V5 + Opus + 额度可用 |
| 预计 Anlas | `预计 5` | 能算出确定金额 | 基础费用确定，加上 Precise Reference 的每张 5 Anlas |
| 待确认 | `费用待确认` | 当前组合的费用以 NovelAI 为准 | 定价未校准、等级未知、参数不支持 |

**已实测确认的两条规则**（都用生成前后的余额观测反查过）：

- **Image2Img 不额外收费**：带起点图生成一次，余额 407 → 407；
- **Precise Reference 每张 5 Anlas**：带 1 张参考图生成一次，余额 407 → 402。
