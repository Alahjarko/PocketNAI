...
打开后，生成期间画廊里的"生成中"卡片会实时显示**正在长出来的画面**（左下角带"生成中 N%"），
画完之后被正式结果替换。

- 中间图**只用于预览、不写入历史**，生成结束即清理；最终图仍是普通 PNG，与其他生成完全一致；
- **默认关闭**：功能已验证可用，但拿到的是服务端的**原始采样帧**，观感与官方网页版
  （对中间图做过平滑处理）不同，且生成只要几秒时画面一闪而过。想试可以在设置页打开；
- **失败不自动重试**（与普通生成同一原则）。连续 3 次流式失败会自动关闭该功能并提示
  "下次生成将使用普通方式"，可以在设置页重新打开。
...
| 流式中间预览（生成中实时显示中间图 + 进度） | ✅ 已实现但**默认关闭**（2026-09-16 真机验证；观感与官方网页版有差距，设置页可开启） |
...
5. **流式中间预览已实现但默认关闭**（2026-09-16 用户实测后决定）：中间图是 JPEG、只用于预览、不写入历史；观感与官方网页版有差距，设置页可手动开启；连续 3 次失败会自动关闭。
