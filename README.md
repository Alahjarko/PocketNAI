...
- **所见即所提交**：底图在进入重绘时按输出尺寸裁切并落盘，编辑器显示的就是提交图，
  手指涂的坐标与提交的蒙版像素严格对应；
- **模型限制：只有 Full 档位能用**。服务端拒绝 Curated 的 infill
  （`Model nai-diffusion-4-5-curated doesn't support action infill`）；
  试过让 Curated 走 `img2img` + 蒙版，请求成功但**蒙版被完全忽略**
  （定量验证：涂了 3.55% 的区域，出图却有 52.6% 的像素变化、包围盒是整张图）。
  官方**网页**上 Curated 能重绘，但公开 API 的能力边界不同 —— 界面上会说明并让你切换模型；
...
| 局部重绘 Inpaint（**仅 Full 档位**，蒙版编辑器 + 扩张实时预览） | ⚠️ 界面与请求已实现，链路待真机最终验证 |
