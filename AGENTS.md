...
- **宽屏（横屏/平板）分两档**（2026-09-21 用户点名）：≥600dp 时导航从底部
  `NavigationBar` 换成最左侧的 `NavigationRail`（`PocketNaiApp` 里按 `BoxWithConstraints`
  宽度切换）；内容宽度 ≥720dp（屏幕约 800dp 起）时再进一步，悬浮层整个取消，变成
