 ...
 *
 * ## 与官方前端的两处差异（2026-09-18 都已核对，不影响数字）
 * 1. 官方前端总是显式发送 `sm` / `sm_dyn`，但 V4/V4.5/V5 的模型能力表里**没有 smea 能力位**，
 *    前端在组装请求时会把这两个字段删掉 —— 这四个模型的 SMEA 倍率恒为 1，
 *    [AnlasPricingContext.smeaMultiplier] 保持 1.0 就是官方行为（技术决策记录 §30.3）。
 *    式子里的 1.2 / 1.4 只服务于 V1–V3，与我们无关。
 * 2. 免费判定读的 `characterRef` 是个**死字段**（bundle 里只读不写），多角色提示词
 *    （`char_captions`）在计价组装里也**没有附加费项** —— 两者都不改价格（§30.2）。
 */
